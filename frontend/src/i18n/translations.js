const translations = {
  pt: {
    lang: { pt: 'Português', en: 'English', es: 'Español' },
    footer: { rights: 'Todos os direitos reservados.' },
    placeholder: { comingSoon: 'Esta página será implementada em breve.' },

    config: {
      filters: 'Filtro',
      showFilters: 'Mostrar Filtros',
      company: 'Empresa',
      companyPlaceholder: 'Buscar empresa...',
      noResults: 'Nenhum resultado encontrado',
      documentType: 'Tipo de Documento',
      selectDocType: 'Selecione...',
      companyCode: 'Código da Empresa',
      selectCode: 'Selecione...',
      filter: 'Filtrar',
      saveAll: 'Salvar',
      saveIndividual: 'Salvar individual',
      savedOk: 'salvo com sucesso.',
      saveError: 'Erro ao salvar.',
      constantName: 'Descrição',
      value: 'Valor',
    },

    rulePrint: {
      rules: 'Regras de Impressão',
      addRule: 'Adicionar Regra',
      editRule: 'Editar Regra',
      back: 'Voltar',
      ruleName: 'Nome da Regra',
      printerName: 'Impressora',
      process: 'Processo',
      paperSource: 'Fonte de Papel',
      copies: 'Cópias',
      storePdf: 'Armazenar PDF',
      reportLayout: 'Layout do Relatório',
      pclTemplate: 'Template PCL',
      printerAgent: 'Agente de Impressão',
      xpathCondition: 'Condição XPath',
      xpathPlaceholder: 'Ex: //ide/tpEmis[text()="5"]',
      fileNameOutput: 'Nome do Arquivo',
      selectFile: 'Selecionar arquivo...',
      downloadLayout: 'Download Layout',
      downloadPcl: 'Download PCL',
      save: 'Salvar',
      actions: 'Ações',
      edit: 'Editar',
      deleteRule: 'Excluir',
      noRules: 'Nenhuma regra encontrada.',
      confirmDelete: 'Confirmar Exclusão',
      confirmDeleteMsg: 'Deseja remover a regra',
      cancel: 'Cancelar',
      requiredFields: 'Preencha os campos obrigatórios: Nome da Regra, Impressora e Processo.',
      ruleCreated: 'Regra criada com sucesso.',
      ruleUpdated: 'Regra atualizada com sucesso.',
      ruleDeleted: 'Regra removida com sucesso.',
      ruleExists: 'Já existe uma regra com este nome.',
      saveError: 'Erro ao salvar.',
      company: 'Empresa',
      documentType: 'Tipo de Documento',
      companyCode: 'Código da Empresa',
    },

    ruleDist: {
      rules: 'Regras de Distribuição', addRule: 'Adicionar Regra', editRule: 'Editar Regra', back: 'Voltar', save: 'Salvar',
      ruleName: 'Nome da Regra', process: 'Processo', attachments: 'Anexos', fileNameToAttach: 'Nome do Arquivo', extensionsToAttach: 'Extensões', xpathCondition: 'Condição XPath',
      tabGeneral: 'Regra de Distribuição', emailTo: 'E-mail Para', subject: 'Assunto', templateFile: 'Arquivo Template',
      wsUrl: 'URL do WebService', wsHeaderTemplate: 'Template Header', wsBodyTemplate: 'Template Body', wsTransformationTemplate: 'Template Transformação', wsContentTagName: 'Tag de Conteúdo',
      ftpHostName: 'Host', ftpPortNumber: 'Porta', ftpProtocol: 'Protocolo', ftpUserName: 'Usuário', ftpPassword: 'Senha', ftpPrivateKeyFile: 'Chave Privada', ftpRemoteDirectory: 'Diretório Remoto',
      as2From: 'AS2 De', as2To: 'AS2 Para', as2Url: 'URL AS2', as2PublicKeyFile: 'Chave Pública', as2FromPublicKeyFile: 'Chave Pública Origem',
      download: 'Download', actions: 'Ações', edit: 'Editar', deleteRule: 'Excluir', noRules: 'Nenhuma regra encontrada.',
      confirmDelete: 'Confirmar Exclusão', confirmDeleteMsg: 'Deseja remover a regra', cancel: 'Cancelar',
      requiredFields: 'Preencha os campos obrigatórios.', ruleCreated: 'Regra criada. Configure os canais nas abas.', ruleDeleted: 'Regra removida.', ruleExists: 'Regra já existe.', saveError: 'Erro ao salvar.', tabSaved: 'Aba salva com sucesso.',
      company: 'Empresa', documentType: 'Tipo de Documento', companyCode: 'Código da Empresa',
    },

    partner: {
      partners: 'Parceiros', addPartner: 'Adicionar', editPartner: 'Editar Parceiro', back: 'Voltar', save: 'Salvar',
      name: 'Nome', actions: 'Ações', edit: 'Editar', deletePartner: 'Excluir',
      noPartners: 'Nenhum parceiro encontrado.', searchPlaceholder: 'Buscar parceiro...',
      loading: 'Carregando...',
      confirmDelete: 'Confirmar Exclusão', confirmDeleteMsg: 'Deseja remover o parceiro', cancel: 'Cancelar',
      nameRequired: 'O campo Nome é obrigatório.',
      created: 'Parceiro criado com sucesso.', updated: 'Parceiro atualizado com sucesso.', deleted: 'Parceiro removido com sucesso.',
      saveError: 'Erro ao salvar.', deleteFkError: 'Não foi possível excluir: parceiro possui dependências.',
      hasDependencies: 'Este parceiro possui dependências:', depCompanies: 'empresa(s)', depUsers: 'usuário(s)', depWorkflows: 'workflow(s)',
    },

    company: {
      companies: 'Empresas', addCompany: 'Adicionar', editCompany: 'Editar Empresa', back: 'Voltar', save: 'Salvar', add: 'Adicionar',
      nameLabel: 'Nome', creationDate: 'Data Criação', partnerNameLabel: 'Parceiro', searchCodeLabel: 'Código Busca', actions: 'Ações', edit: 'Editar', deleteLabel: 'Excluir', cancel: 'Cancelar',
      noCompanies: 'Nenhuma empresa encontrada.', searchPlaceholder: 'Buscar empresa...', loading: 'Carregando...',
      confirmDelete: 'Confirmar Exclusão', confirmDeleteMsg: 'Deseja remover esta empresa?',
      requiredFields: 'Company ID e Nome são obrigatórios.', created: 'Empresa criada.', updated: 'Empresa atualizada.', deleted: 'Empresa removida.', saveError: 'Erro ao salvar.',
      tabCompany: 'Empresa', tabCompanyCode: 'Código Empresa', tabCompanyBranch: 'Filial',
      documentType: 'Tipo Documento', codeLabel: 'Código', selectDocType: 'Selecione o tipo...',
      companyCodePlaceholder: 'Código...', noCodes: 'Nenhum código cadastrado.', codeAdded: 'Código adicionado.', codeDeleted: 'Código removido.',
      confirmDeleteCodeMsg: 'Excluir código (e todas as configurações associadas)?', cascadeWarning: 'Constantes, regras de impressão, distribuição e contingência serão excluídas.',
      branchCodePlaceholder: 'Código filial...', branchNamePlaceholder: 'Nome filial...', branchName: 'Nome',
      noBranches: 'Nenhuma filial cadastrada.', branchAdded: 'Filial adicionada.', branchUpdated: 'Filial atualizada.', branchDeleted: 'Filial removida.',
      confirmDeleteBranchMsg: 'Deseja remover esta filial?',
    },

    userPage: {
      users: 'Usuários', addUser: 'Adicionar', editUser: 'Editar Usuário', back: 'Voltar', save: 'Salvar',
      group: 'Grupo', name: 'Nome', phone: 'Telefone', mobile: 'Celular', password: 'Senha', confirmPassword: 'Confirmar Senha',
      approved: 'Aprovado', lockedOut: 'Bloqueado', lastLogin: 'Último Login', actions: 'Ações', edit: 'Editar', deleteUser: 'Excluir',
      lock: 'Bloquear', unlock: 'Desbloquear', yes: 'Sim', no: 'Não',
      noUsers: 'Nenhum usuário encontrado.', searchPlaceholder: 'Buscar usuário...', loading: 'Carregando...',
      confirmDelete: 'Confirmar Exclusão', confirmDeleteMsg: 'Deseja excluir o usuário',
      confirmLock: 'Bloquear Usuário', confirmLockMsg: 'Deseja bloquear o usuário',
      confirmUnlock: 'Desbloquear Usuário', confirmUnlockMsg: 'Deseja desbloquear o usuário',
      cancel: 'Cancelar', confirm: 'Confirmar', remove: 'Remover',
      created: 'Usuário criado.', updated: 'Usuário atualizado.', deleted: 'Usuário excluído.', locked: 'Usuário bloqueado.', unlocked: 'Usuário desbloqueado.',
      passwordHint: '10-24 caracteres: 1 maiúscula, 1 minúscula, 1 número, 1 especial',
      tabUser: 'Usuário', tabCompanyUser: 'Empresas do Usuário',
      selectCompany: 'Selecione empresa...', addCompany: 'Adicionar', companyName: 'Empresa', noCompanies: 'Nenhuma empresa associada.',
      companyAdded: 'Empresa associada.', companyRemoved: 'Empresa removida.', companyAlreadyAssigned: 'Empresa já associada.',
      errors: { EMAIL_REQUIRED: 'E-mail obrigatório.', EMAIL_INVALID: 'E-mail inválido.', EMAIL_DUPLICATE: 'E-mail já em uso.', NAME_REQUIRED: 'Nome obrigatório.', PHONE_REQUIRED: 'Telefone obrigatório.', PASSWORD_WEAK: 'Senha não atende requisitos.', PASSWORD_MISMATCH: 'Senhas não conferem.' },
    },

    businessGroup: {
      groups: 'Grupos Empresariais', addGroup: 'Adicionar', editGroup: 'Editar Grupo', back: 'Voltar', save: 'Salvar',
      name: 'Nome', description: 'Descrição', actions: 'Ações', edit: 'Editar', loading: 'Carregando...', noGroups: 'Nenhum grupo encontrado.',
      tabGroup: 'Grupo Empresarial', tabAuthorizations: 'Autorizações',
      selectAuthorization: 'Selecione autorização...', addAuth: 'Adicionar', removeAuth: 'Remover',
      authName: 'Nome', authDescription: 'Descrição', noAuthorizations: 'Nenhuma autorização atribuída.',
      allAssigned: 'Todas as autorizações já foram atribuídas.',
      confirmRemoveAuth: 'Remover Autorização', confirmRemoveAuthMsg: 'Deseja remover a autorização', cancel: 'Cancelar',
      requiredFields: 'Nome e Descrição são obrigatórios.', created: 'Grupo criado.', updated: 'Grupo atualizado.', saveError: 'Erro ao salvar.',
      authAdded: 'Autorização adicionada.', authRemoved: 'Autorização removida.',
    },

    menu: {
      dashboard: 'Dashboard',
      documents: 'Documentos',
      processes: 'Processos',
      configuration: 'Configuração',
      configGeneral: 'Geral',
      configRulePrint: 'Regra de Impressão',
      configRuleDistribution: 'Regra de Distribuição',
      security: 'Segurança',
      secPartner: 'Parceiros',
      secCompanies: 'Empresas',
      secUsers: 'Usuários',
      secBusinessGroup: 'Grupo Empresarial',
      logReception: 'Log de Recepção',
      idocGeneration: 'Geração de IDoc',
      utilities: 'Úteis',
      utilSearch: 'Busca e Ações',
      utilExport: 'Exportação',
      utilCountry: 'Operações por País',
      utilQueue: 'Gerenciar Filas',
    },
    proc: {
      filters: 'Filtros de Busca', dailyDb: 'Base Diária', historicalDb: 'Base Histórica', docType: 'Tipo de Documento',
      period: 'Período', dateFrom: 'Data criação de', dateTo: 'Data criação até', data: 'Dados', serie: 'Série', numero: 'Número', file: 'Arquivo', filePlaceholder: 'IDs (um por linha)...',
      or: 'OU', search: 'Buscar', searchDone: 'Busca concluída', found: 'Encontrados', documents: 'documentos', noDocuments: 'Nenhum documento encontrado.',
      confirmProcess: 'Processar', colLeft: 'Processos (Coluna 1)', colRight: 'Processos (Coluna 2)',
      queueLabel: 'Fila de Destino', changeQueueTo: 'Alterar QueueName para', sendToQueue: 'Enviar para Fila', changeTpEmis: 'Mudar Tipo Emissão',
      exportObjects: 'Exportar Objetos', kelloggIndex: 'Processa Kellogg', cfdiApproval: 'CFDI Approval Msg',
      clearQueue: 'Clear TFMessageQueue2', downloadNfe: 'Download NFe', rebuildZips: 'Rebuild Zips', grainger: 'Processa Grainger',
      envioDte: 'Processa EnvioDTE', exportAttach: 'Exportar Anexo', dlReturns: 'Descarregar Retornos',
      purge: 'Apagar Documentos', nfePdf: 'NFe PDF Recep', chep: 'Processa Chep', uuid: 'Processa UUID', fixTimbre: 'Fix Timbre CFDI',
      logTitle: 'Console de Processos', clearLog: 'Limpar Log',
    },
    util: {
      searchTitle: 'Busca de Documentos', mode_period: 'Por Período', mode_documentId: 'Por Document ID', mode_data: 'Por Dados', mode_file: 'Por Arquivo',
      dateFrom: 'Data de', dateTo: 'Data até', cnpj: 'CNPJ Emitente', serie: 'Série', numero: 'Número', fileIds: 'IDs (um por linha)', fileIdsPlaceholder: 'DOC-001\nDOC-002\n...',
      docType: 'Tipo Documento', all: 'Todos', historical: 'Histórico', search: 'Buscar', results: 'Resultados', selected: 'selecionados',
      noDocsSelected: 'Nenhum documento selecionado.', creationDate: 'Data Criação',
      sendToQueue: 'Enviar para Fila', purge: 'Apagar Documentos', reprint: 'Reimpressão PDF', downloadReturns: 'Descarregar Retornos',
      log: 'Console de Log',
      exportTitle: 'Exportação de Documentos', documentIds: 'Document IDs', idsPlaceholder: 'Cole os IDs (um por linha)...',
      noIds: 'Nenhum ID informado.',
      exportActions: 'Ações de Exportação',
      exportObjectSpace: 'Exportar ObjectSpace', exportObjectSpaceDesc: 'Exporta o ObjectSpace completo como XML.',
      exportAttachments: 'Exportar Anexos', exportAttachmentsDesc: 'Exporta anexos (XML, PDF, TXT) dos documentos.',
      allAttachments: 'Todos os Anexos',
      backupExport: 'Backup Completo', backupExportDesc: 'Exporta anexos em estrutura de diretórios organizada.',
      exportIndex: 'Índice Tabulado', exportIndexDesc: 'Gera arquivo de índice tabulado para CFDI.',
      changeEmissionType: 'Mudar Tipo de Emissão', changeEmissionTypeDesc: 'Altera tpEmis da NFe, recalcula chave, remove assinatura.', justification: 'Justificativa de contingência...',
      chepIndex: 'Processar CHEP Index', chepIndexDesc: 'Aplica template de mapeamento CHEP e gera anexo Index.',
      fixTimbreCfdi: 'Fix Timbre CFDI', fixTimbreCfdiDesc: 'Corrige TimbreFiscalDigital no ComprobantePrint.',
      importUuid: 'Importar UUID XMLs', importUuidDesc: 'Importa XMLs com UUID do diretório.',
      cfdiApproval: 'CFDI Approval Message', cfdiApprovalDesc: 'Gera mensagem de aprovação para CFDI.',
      fixEnvioDte: 'Fix EnvioDTE', fixEnvioDteDesc: 'Reconstrói envelope EnvioDTE e assina digitalmente.',
      execute: 'Executar',
      extractObjectSpace: 'Extrair ObjectSpace', extractObjectSpaceDesc: 'Extrai ObjectSpace de uma fila de mensagens.',
      queueName: 'Nome da Fila', count: 'Quantidade', extract: 'Extrair',
      clearQueueTitle: 'Limpar Fila', clearQueueDesc: 'Remove todas as mensagens de uma fila.', clearQueue: 'Limpar Fila',
    },

    topbar: {
      search: 'Buscar...',
      newRecord: 'Novo Registro',
    },

    dashboard: {
      title: 'Visão Geral',
      subtitle: 'Resumo de processamento de documentos dos últimos 7 dias.',
    },
    dash: {
      totalProcessed: 'Documentos Processados', totalErrors: 'Erros (7 dias)', successRate: 'Taxa de Sucesso', avgTime: 'Tempo Médio',
      docsPerDay: 'Documentos por Dia (7 dias)', success: 'Sucesso', errors: 'Erros',
      successRateTitle: 'Taxa de Sucesso Global',
      errorsByCompany: 'Erros por Empresa (7 dias)', company: 'Empresa', other: 'Outros',
      errorsByDocType: 'Erros por Tipo de Documento',
      topDocTypes: 'Volume por Tipo de Documento', docType: 'Tipo', processed: 'Processados', successCol: 'Sucesso', errorsCol: 'Erros', rate: 'Taxa',
      recentErrors: 'Erros Recentes',
    },

    login: {
      title: 'Bem-vindo de volta',
      subtitle: 'Faça login para acessar o painel.',
      user: 'Usuário',
      userPlaceholder: 'Digite seu usuário',
      password: 'Senha',
      passwordPlaceholder: 'Digite sua senha',
      submit: 'Entrar',
      rememberMe: 'Lembrar-me',
      forgotPassword: 'Esqueceu a senha?',
      errorEmpty: 'Preencha o usuário e a senha.',
      errorInvalid: 'Usuário ou senha inválidos.',
    },

    docs: {
      filtersTitle: 'Filtros de Documentos',
      clearFilters: 'Limpar Filtros',
      documentType: 'Tipo de Documento',
      selectDocType: 'Selecione o tipo...',
      process: 'Processo',
      selectProcess: 'Selecione...',
      outbound: 'Documento Enviado (Outbound)',
      inbound: 'Documento Recebido (Inbound)',
      action: 'Ação',
      selectAction: 'Selecione...',
      situation: 'Situação',
      all: 'Todos',
      processing: 'Processando',
      finishedSuccess: 'Concluído com Sucesso',
      finishedError: 'Concluído com Erro',
      historic: 'Buscar no histórico',
      search: 'Buscar',
      dynamicFilters: 'Filtros Específicos do Documento',
      from: 'De',
      to: 'Até',
      radioIgnore: 'Ignorar',
      radioApproved: 'Aprovado',
      radioRejected: 'Rejeitado',
      radioNotContains: 'Não Contém',
      results: 'Resultados',
      noResults: 'Nenhum documento encontrado.',
      createdAt: 'Data de Criação',
      showFilters: 'Mostrar Filtros',
      batchActions: 'Ações em Lote',
      exportExcel: 'Exportar Excel',
      noDocSelected: 'Nenhum documento selecionado.',
    },
    docTypes: {
      arAfipLocalInvoice: 'AR - AFIP Fatura Local',
      arAfipLocalInvoiceMtx: 'AR - AFIP Fatura Local MTX',
      arAfipExportInvoice: 'AR - AFIP Fatura Exportação',
      brNfe: 'BR - NF-e (Nota Fiscal Eletrônica)',
      brNf3e: 'BR - NF3e (Energia Elétrica)',
      brCte: 'BR - CT-e (Conhecimento Transporte)',
      brMdfe: 'BR - MDF-e (Manifesto)',
      brInutNfe: 'BR - Inutilização NF-e',
      brReprint: 'BR - Reimpressão NF-e',
      brNfseCancelamento: 'BR - NFS-e Cancelamento',
      clDte: 'CL - DTE (Documento Tributário)',
      clResultadoEnvio: 'CL - Resultado Envio',
      clResultadoEnvioLibro: 'CL - Resultado Envio Libro',
      clLibroCompraVenta: 'CL - Libro de Compra/Venta',
      crFactura: 'CR - Factura Electrónica',
      crTiquete: 'CR - Tiquete Electrónico',
      crNotaDebito: 'CR - Nota Débito Electrónica',
      ecFactura: 'EC - Factura',
      ecNotaDebito: 'EC - Nota de Débito',
      ecNotaCredito: 'EC - Nota de Crédito',
    },
    docFilters: {
      cuitContribuyente: 'CUIT Contribuyente',
      cuitReceptor: 'CUIT Receptor',
      factura: 'Factura',
      fechaComprobante: 'Fecha Comprobante',
      puntoDeVentas: 'Punto de Ventas',
      tipoComprobante: 'Tipo Comprobante',
      puntoVenta: 'Punto Venta',
      codigoTipoAutorizacion: 'Código Tipo Autorización',
      identificador: 'Identificador',
      puntoDeVentasInt: 'Punto de Ventas',
      cnpjEmissor: 'CNPJ Emissor',
      cnpjReceptor: 'CNPJ Receptor',
      chaveAcesso: 'Chave de Acesso',
      serie: 'Série',
      numeroDocumento: 'Número do Documento',
      codigoNumerico: 'Código Numérico',
      ambiente: 'Ambiente',
      dataEmissao: 'Data de Emissão',
      statusSefaz: 'Status SEFAZ',
      statusSefaz2: 'Status SEFAZ 2',
      numMdf: 'Número MDF',
      dtHrEmissao: 'Data/Hora Emissão',
      numInicial: 'Número Inicial',
      cnpj: 'CNPJ',
      cnpjPrestador: 'CNPJ Prestador',
      inscricaoMunicipal: 'Inscrição Municipal',
      numNfse: 'Número NFS-e',
      codigoCancelamento: 'Código Cancelamento',
      rutEmisor: 'RUT Emisor',
      tipo: 'Tipo',
      folio: 'Folio',
      codigoDocumento: 'Código Documento',
      fechaEmision: 'Fecha Emisión',
      rutEnvia: 'RUT Envia',
      trackingId: 'Tracking ID',
      referencia: 'Referência',
      tipoOperacion: 'Tipo Operação',
      tipoLibro: 'Tipo Libro',
      tipoEnvio: 'Tipo Envio',
      emisorNumeroCedula: 'Emisor Número Cédula',
      emisorNombre: 'Emisor Nombre',
      receptorNumeroCedula: 'Receptor Número Cédula',
      receptorNombre: 'Receptor Nombre',
      clave: 'Clave',
      numeroConsecutivo: 'Número Consecutivo',
      rucEmisor: 'RUC Emisor',
      secuencial: 'Secuencial',
    },
    docActions: {
      actionAuthorize: 'Autorizar',
      actionSend: 'Enviar',
      actionCancel: 'Cancelar',
      actionReceive: 'Receber',
    },
    detail: {
      loading: 'Carregando...',
      back: 'Voltar',
      refresh: 'Atualizar',
      actions: 'Ações',
      documentData: 'Dados do Documento',
      semaphore: 'Semáforo',
      tabHistory: 'Histórico',
      tabAttachments: 'Anexos',
      tabErrors: 'Erros',
      tabAudit: 'Auditoria',
      tabSupport: 'Suporte',
      tabFlags: 'Flags',
      tabMessages: 'Mensagens',
      creationDate: 'Data de Criação',
      creationDateUTC: 'Data de Criação (UTC)',
      attachmentName: 'Nome do Anexo',
      download: 'Download',
      method: 'Método',
      message: 'Mensagem',
      username: 'Usuário',
      actionDesc: 'Ação',
      description: 'Descrição',
      flagName: 'Nome',
      flagValue: 'Valor',
      documentInfo: 'Informações do Documento',
      supportActions: 'Ações de Suporte',
      queueNamePlaceholder: 'Nome da fila...',
      sendToQueue: 'Enviar para Fila',
      exportObjectSpace: 'Exportar ObjectSpace',
      deleteDocument: 'Excluir Documento',
      confirmDelete: 'Confirmar Exclusão',
      confirmDeleteMsg: 'Tem certeza? Esta ação não pode ser desfeita. Informe a justificativa.',
      justification: 'Justificativa',
      justificationPlaceholder: 'Motivo da exclusão...',
      cancel: 'Cancelar',
      copied: 'Copiado!',
      view: 'Visualizar',
      copy: 'Copiar',
    },

    crud: {
      title: 'CRUD Demo',
      subtitle: 'Demonstração de formulários, tabelas, botões, cards e demais componentes.',
      totalRecords: 'Total Registros',
      activeLabel: 'Ativos',
      inactiveLabel: 'Inativos',
      monthlyPayroll: 'Folha Mensal',
      newRecord: 'Novo Registro',
      editRecord: 'Editar Registro',
      cancelEdit: 'Cancelar Edição',
      name: 'Nome',
      email: 'E-mail',
      phone: 'Telefone',
      role: 'Cargo',
      department: 'Departamento',
      salary: 'Salário',
      startDate: 'Data de Início',
      bio: 'Biografia',
      activeToggle: 'Ativo',
      addRecord: 'Adicionar Registro',
      saveChanges: 'Salvar Alterações',
      clear: 'Limpar',
      selectPlaceholder: 'Selecione...',
      namePlaceholder: 'Nome completo',
      emailPlaceholder: 'email@empresa.com',
      phonePlaceholder: '(00) 00000-0000',
      salaryPlaceholder: '0,00',
      bioPlaceholder: 'Breve descrição do colaborador...',
      buttonsAndBadges: 'Botões & Badges',
      records: 'Registros',
      filterPlaceholder: 'Filtrar por nome, e-mail ou cargo...',
      actions: 'Ações',
      noRecords: 'Nenhum registro encontrado.',
      statusActive: 'Ativo',
      statusInactive: 'Inativo',
      previous: '← Anterior',
      next: 'Próximo →',
      confirmDelete: 'Confirmar Exclusão',
      confirmDeleteMsg: 'Tem certeza que deseja excluir o registro de',
      cannotUndo: 'Esta ação não pode ser desfeita.',
      cancel: 'Cancelar',
      delete: 'Excluir',
      edit: 'Editar',
      badgeNew: 'Novo',
      badgePending: 'Pendente',
      requiredFields: 'Preencha os campos obrigatórios: Nome, E-mail e Cargo.',
      recordUpdated: 'Registro de "{name}" atualizado com sucesso.',
      recordCreated: 'Registro de "{name}" criado com sucesso.',
      recordDeleted: 'Registro de "{name}" removido.',
      roles: ['Desenvolvedora', 'Designer', 'Gerente de Projetos', 'Analista de Dados', 'QA Engineer', 'DevOps', 'Product Owner'],
      departments: ['Engenharia', 'Produto', 'Operações', 'Business Intelligence', 'Marketing', 'RH'],
    },
  },

  en: {
    lang: { pt: 'Português', en: 'English', es: 'Español' },
    footer: { rights: 'All rights reserved.' },
    placeholder: { comingSoon: 'This page will be implemented soon.' },

    config: {
      filters: 'Filter',
      showFilters: 'Show Filters',
      company: 'Company',
      companyPlaceholder: 'Search company...',
      noResults: 'No results found',
      documentType: 'Document Type',
      selectDocType: 'Select...',
      companyCode: 'Company Code',
      selectCode: 'Select...',
      filter: 'Filter',
      saveAll: 'Save',
      saveIndividual: 'Save individual',
      savedOk: 'saved successfully.',
      saveError: 'Error saving.',
      constantName: 'Description',
      value: 'Value',
    },

    rulePrint: {
      rules: 'Print Rules',
      addRule: 'Add Rule',
      editRule: 'Edit Rule',
      back: 'Back',
      ruleName: 'Rule Name',
      printerName: 'Printer Name',
      process: 'Process',
      paperSource: 'Paper Source',
      copies: 'Copies',
      storePdf: 'Store PDF',
      reportLayout: 'Report Layout',
      pclTemplate: 'PCL Template',
      printerAgent: 'Printer Agent',
      xpathCondition: 'XPath Condition',
      xpathPlaceholder: 'Ex: //ide/tpEmis[text()="5"]',
      fileNameOutput: 'File Name',
      selectFile: 'Select file...',
      downloadLayout: 'Download Layout',
      downloadPcl: 'Download PCL',
      save: 'Save',
      actions: 'Actions',
      edit: 'Edit',
      deleteRule: 'Delete',
      noRules: 'No rules found.',
      confirmDelete: 'Confirm Deletion',
      confirmDeleteMsg: 'Do you want to remove the rule',
      cancel: 'Cancel',
      requiredFields: 'Please fill required fields: Rule Name, Printer Name and Process.',
      ruleCreated: 'Rule created successfully.',
      ruleUpdated: 'Rule updated successfully.',
      ruleDeleted: 'Rule removed successfully.',
      ruleExists: 'A rule with this name already exists.',
      saveError: 'Error saving.',
      company: 'Company',
      documentType: 'Document Type',
      companyCode: 'Company Code',
    },

    ruleDist: {
      rules: 'Distribution Rules', addRule: 'Add Rule', editRule: 'Edit Rule', back: 'Back', save: 'Save',
      ruleName: 'Rule Name', process: 'Process', attachments: 'Attachments', fileNameToAttach: 'File Name', extensionsToAttach: 'Extensions', xpathCondition: 'XPath Condition',
      tabGeneral: 'Rule of Distribution', emailTo: 'Email To', subject: 'Subject', templateFile: 'Template File',
      wsUrl: 'WebService URL', wsHeaderTemplate: 'Header Template', wsBodyTemplate: 'Body Template', wsTransformationTemplate: 'Transformation Template', wsContentTagName: 'Content Tag Name',
      ftpHostName: 'Host', ftpPortNumber: 'Port', ftpProtocol: 'Protocol', ftpUserName: 'Username', ftpPassword: 'Password', ftpPrivateKeyFile: 'Private Key', ftpRemoteDirectory: 'Remote Directory',
      as2From: 'AS2 From', as2To: 'AS2 To', as2Url: 'AS2 URL', as2PublicKeyFile: 'Public Key', as2FromPublicKeyFile: 'From Public Key',
      download: 'Download', actions: 'Actions', edit: 'Edit', deleteRule: 'Delete', noRules: 'No rules found.',
      confirmDelete: 'Confirm Deletion', confirmDeleteMsg: 'Do you want to remove the rule', cancel: 'Cancel',
      requiredFields: 'Please fill required fields.', ruleCreated: 'Rule created. Configure channels in the tabs.', ruleDeleted: 'Rule removed.', ruleExists: 'Rule already exists.', saveError: 'Error saving.', tabSaved: 'Tab saved successfully.',
      company: 'Company', documentType: 'Document Type', companyCode: 'Company Code',
    },

    partner: {
      partners: 'Partners', addPartner: 'Add', editPartner: 'Edit Partner', back: 'Back', save: 'Save',
      name: 'Name', actions: 'Actions', edit: 'Edit', deletePartner: 'Delete',
      noPartners: 'No partners found.', searchPlaceholder: 'Search partner...',
      loading: 'Loading...',
      confirmDelete: 'Confirm Deletion', confirmDeleteMsg: 'Do you want to remove the partner', cancel: 'Cancel',
      nameRequired: 'Name is required.',
      created: 'Partner created successfully.', updated: 'Partner updated successfully.', deleted: 'Partner removed successfully.',
      saveError: 'Error saving.', deleteFkError: 'Cannot delete: partner has dependencies.',
      hasDependencies: 'This partner has dependencies:', depCompanies: 'company(ies)', depUsers: 'user(s)', depWorkflows: 'workflow(s)',
    },

    company: {
      companies: 'Companies', addCompany: 'Add', editCompany: 'Edit Company', back: 'Back', save: 'Save', add: 'Add',
      nameLabel: 'Name', creationDate: 'Creation Date', partnerNameLabel: 'Partner', searchCodeLabel: 'Search Code', actions: 'Actions', edit: 'Edit', deleteLabel: 'Delete', cancel: 'Cancel',
      noCompanies: 'No companies found.', searchPlaceholder: 'Search company...', loading: 'Loading...',
      confirmDelete: 'Confirm Deletion', confirmDeleteMsg: 'Do you want to remove this company?',
      requiredFields: 'Company ID and Name are required.', created: 'Company created.', updated: 'Company updated.', deleted: 'Company removed.', saveError: 'Error saving.',
      tabCompany: 'Company', tabCompanyCode: 'Company Code', tabCompanyBranch: 'Company Branch',
      documentType: 'Document Type', codeLabel: 'Code', selectDocType: 'Select type...',
      companyCodePlaceholder: 'Code...', noCodes: 'No codes registered.', codeAdded: 'Code added.', codeDeleted: 'Code removed.',
      confirmDeleteCodeMsg: 'Delete code (and all associated configurations)?', cascadeWarning: 'Constants, print rules, distribution rules and contingency rules will be deleted.',
      branchCodePlaceholder: 'Branch code...', branchNamePlaceholder: 'Branch name...', branchName: 'Name',
      noBranches: 'No branches registered.', branchAdded: 'Branch added.', branchUpdated: 'Branch updated.', branchDeleted: 'Branch removed.',
      confirmDeleteBranchMsg: 'Do you want to remove this branch?',
    },

    userPage: {
      users: 'Users', addUser: 'Add', editUser: 'Edit User', back: 'Back', save: 'Save',
      group: 'Group', name: 'Name', phone: 'Phone', mobile: 'Mobile', password: 'Password', confirmPassword: 'Confirm Password',
      approved: 'Approved', lockedOut: 'Locked', lastLogin: 'Last Login', actions: 'Actions', edit: 'Edit', deleteUser: 'Delete',
      lock: 'Lock', unlock: 'Unlock', yes: 'Yes', no: 'No',
      noUsers: 'No users found.', searchPlaceholder: 'Search user...', loading: 'Loading...',
      confirmDelete: 'Confirm Deletion', confirmDeleteMsg: 'Do you want to delete user',
      confirmLock: 'Lock User', confirmLockMsg: 'Do you want to lock user',
      confirmUnlock: 'Unlock User', confirmUnlockMsg: 'Do you want to unlock user',
      cancel: 'Cancel', confirm: 'Confirm', remove: 'Remove',
      created: 'User created.', updated: 'User updated.', deleted: 'User deleted.', locked: 'User locked.', unlocked: 'User unlocked.',
      passwordHint: '10-24 chars: 1 uppercase, 1 lowercase, 1 number, 1 special',
      tabUser: 'User', tabCompanyUser: 'User Companies',
      selectCompany: 'Select company...', addCompany: 'Add', companyName: 'Company', noCompanies: 'No companies assigned.',
      companyAdded: 'Company assigned.', companyRemoved: 'Company removed.', companyAlreadyAssigned: 'Company already assigned.',
      errors: { EMAIL_REQUIRED: 'Email required.', EMAIL_INVALID: 'Invalid email.', EMAIL_DUPLICATE: 'Email already in use.', NAME_REQUIRED: 'Name required.', PHONE_REQUIRED: 'Phone required.', PASSWORD_WEAK: 'Password does not meet requirements.', PASSWORD_MISMATCH: 'Passwords do not match.' },
    },

    businessGroup: {
      groups: 'Business Groups', addGroup: 'Add', editGroup: 'Edit Group', back: 'Back', save: 'Save',
      name: 'Name', description: 'Description', actions: 'Actions', edit: 'Edit', loading: 'Loading...', noGroups: 'No groups found.',
      tabGroup: 'Business Group', tabAuthorizations: 'Authorizations',
      selectAuthorization: 'Select authorization...', addAuth: 'Add', removeAuth: 'Remove',
      authName: 'Name', authDescription: 'Description', noAuthorizations: 'No authorizations assigned.',
      allAssigned: 'All authorizations have been assigned.',
      confirmRemoveAuth: 'Remove Authorization', confirmRemoveAuthMsg: 'Do you want to remove authorization', cancel: 'Cancel',
      requiredFields: 'Name and Description are required.', created: 'Group created.', updated: 'Group updated.', saveError: 'Error saving.',
      authAdded: 'Authorization added.', authRemoved: 'Authorization removed.',
    },

    menu: {
      dashboard: 'Dashboard',
      documents: 'Documents',
      processes: 'Processes',
      configuration: 'Configuration',
      configGeneral: 'General',
      configRulePrint: 'Rule Print',
      configRuleDistribution: 'Rule of Distribution',
      security: 'Security',
      secPartner: 'Partner',
      secCompanies: 'Companies',
      secUsers: 'Users',
      secBusinessGroup: 'Business Group',
      logReception: 'Log Reception',
      idocGeneration: 'IDoc Generation',
      utilities: 'Utilities',
      utilSearch: 'Search & Actions',
      utilExport: 'Export',
      utilCountry: 'Country Operations',
      utilQueue: 'Queue Management',
    },
    proc: {
      filters: 'Search Filters', dailyDb: 'Daily Database', historicalDb: 'Historical Database', docType: 'Document Type',
      period: 'Period', dateFrom: 'Creation date from', dateTo: 'Creation date to', data: 'Data', serie: 'Series', numero: 'Number', file: 'File', filePlaceholder: 'IDs (one per line)...',
      or: 'OR', search: 'Search', searchDone: 'Search complete', found: 'Found', documents: 'documents', noDocuments: 'No documents found.',
      confirmProcess: 'Process', colLeft: 'Processes (Column 1)', colRight: 'Processes (Column 2)',
      queueLabel: 'Destination Queue', changeQueueTo: 'Change QueueName to', sendToQueue: 'Send to Queue', changeTpEmis: 'Change Emission Type',
      exportObjects: 'Export Objects', kelloggIndex: 'Process Kellogg', cfdiApproval: 'CFDI Approval Msg',
      clearQueue: 'Clear TFMessageQueue2', downloadNfe: 'Download NFe', rebuildZips: 'Rebuild Zips', grainger: 'Process Grainger',
      envioDte: 'Process EnvioDTE', exportAttach: 'Export Attachment', dlReturns: 'Download Returns',
      purge: 'Purge Documents', nfePdf: 'NFe PDF Recep', chep: 'Process Chep', uuid: 'Process UUID', fixTimbre: 'Fix Timbre CFDI',
      logTitle: 'Process Console', clearLog: 'Clear Log',
    },
    util: {
      searchTitle: 'Document Search', mode_period: 'By Period', mode_documentId: 'By Document ID', mode_data: 'By Data', mode_file: 'By File',
      dateFrom: 'Date from', dateTo: 'Date to', cnpj: 'Issuer CNPJ', serie: 'Series', numero: 'Number', fileIds: 'IDs (one per line)', fileIdsPlaceholder: 'DOC-001\nDOC-002\n...',
      docType: 'Document Type', all: 'All', historical: 'Historical', search: 'Search', results: 'Results', selected: 'selected',
      noDocsSelected: 'No documents selected.', creationDate: 'Creation Date',
      sendToQueue: 'Send to Queue', purge: 'Purge Documents', reprint: 'Reprint PDF', downloadReturns: 'Download Returns',
      log: 'Log Console',
      exportTitle: 'Document Export', documentIds: 'Document IDs', idsPlaceholder: 'Paste IDs (one per line)...',
      noIds: 'No IDs provided.',
      exportActions: 'Export Actions',
      exportObjectSpace: 'Export ObjectSpace', exportObjectSpaceDesc: 'Exports the complete ObjectSpace as XML.',
      exportAttachments: 'Export Attachments', exportAttachmentsDesc: 'Exports attachments (XML, PDF, TXT).',
      allAttachments: 'All Attachments',
      backupExport: 'Full Backup', backupExportDesc: 'Exports attachments in organized directory structure.',
      exportIndex: 'Tabbed Index', exportIndexDesc: 'Generates tabbed index file for CFDI.',
      changeEmissionType: 'Change Emission Type', changeEmissionTypeDesc: 'Changes NFe tpEmis, recalculates key, removes signature.', justification: 'Contingency justification...',
      chepIndex: 'Process CHEP Index', chepIndexDesc: 'Applies CHEP mapping template and generates Index attachment.',
      fixTimbreCfdi: 'Fix Timbre CFDI', fixTimbreCfdiDesc: 'Fixes TimbreFiscalDigital in ComprobantePrint.',
      importUuid: 'Import UUID XMLs', importUuidDesc: 'Imports XMLs with UUID from directory.',
      cfdiApproval: 'CFDI Approval Message', cfdiApprovalDesc: 'Generates approval message for CFDI.',
      fixEnvioDte: 'Fix EnvioDTE', fixEnvioDteDesc: 'Rebuilds EnvioDTE envelope and digitally signs.',
      execute: 'Execute',
      extractObjectSpace: 'Extract ObjectSpace', extractObjectSpaceDesc: 'Extracts ObjectSpace from a message queue.',
      queueName: 'Queue Name', count: 'Count', extract: 'Extract',
      clearQueueTitle: 'Clear Queue', clearQueueDesc: 'Removes all messages from a queue.', clearQueue: 'Clear Queue',
    },

    topbar: {
      search: 'Search...',
      newRecord: 'New Record',
    },

    dashboard: {
      title: 'Dashboard Overview',
      subtitle: 'Document processing summary for the last 7 days.',
    },
    dash: {
      totalProcessed: 'Documents Processed', totalErrors: 'Errors (7 days)', successRate: 'Success Rate', avgTime: 'Avg Time',
      docsPerDay: 'Documents per Day (7 days)', success: 'Success', errors: 'Errors',
      successRateTitle: 'Global Success Rate',
      errorsByCompany: 'Errors by Company (7 days)', company: 'Company', other: 'Other',
      errorsByDocType: 'Errors by Document Type',
      topDocTypes: 'Volume by Document Type', docType: 'Type', processed: 'Processed', successCol: 'Success', errorsCol: 'Errors', rate: 'Rate',
      recentErrors: 'Recent Errors',
    },

    login: {
      title: 'Welcome back',
      subtitle: 'Sign in to access the dashboard.',
      user: 'Username',
      userPlaceholder: 'Enter your username',
      password: 'Password',
      passwordPlaceholder: 'Enter your password',
      submit: 'Sign in',
      rememberMe: 'Remember me',
      forgotPassword: 'Forgot password?',
      errorEmpty: 'Please enter username and password.',
      errorInvalid: 'Invalid username or password.',
    },

    docs: {
      filtersTitle: 'Document Filters',
      clearFilters: 'Clear Filters',
      documentType: 'Document Type',
      selectDocType: 'Select type...',
      process: 'Process',
      selectProcess: 'Select...',
      outbound: 'Outbound Document',
      inbound: 'Inbound Document',
      action: 'Action',
      selectAction: 'Select...',
      situation: 'Situation',
      all: 'All',
      processing: 'Processing',
      finishedSuccess: 'Finished with Success',
      finishedError: 'Finished with Error',
      historic: 'Search historic database',
      search: 'Search',
      dynamicFilters: 'Document-Specific Filters',
      from: 'From',
      to: 'To',
      radioIgnore: 'Ignore',
      radioApproved: 'Approved',
      radioRejected: 'Rejected',
      radioNotContains: 'Not Contains',
      results: 'Results',
      noResults: 'No documents found.',
      createdAt: 'Created At',
      showFilters: 'Show Filters',
      batchActions: 'Batch Actions',
      exportExcel: 'Export Excel',
      noDocSelected: 'No document selected.',
    },
    docTypes: {
      arAfipLocalInvoice: 'AR - AFIP Local Invoice',
      arAfipLocalInvoiceMtx: 'AR - AFIP Local Invoice MTX',
      arAfipExportInvoice: 'AR - AFIP Export Invoice',
      brNfe: 'BR - NF-e (Electronic Invoice)',
      brNf3e: 'BR - NF3e (Electric Energy)',
      brCte: 'BR - CT-e (Transport)',
      brMdfe: 'BR - MDF-e (Manifest)',
      brInutNfe: 'BR - NF-e Invalidation',
      brReprint: 'BR - NF-e Reprint',
      brNfseCancelamento: 'BR - NFS-e Cancellation',
      clDte: 'CL - DTE (Tax Document)',
      clResultadoEnvio: 'CL - Sending Result',
      clResultadoEnvioLibro: 'CL - Book Sending Result',
      clLibroCompraVenta: 'CL - Purchase/Sales Book',
      crFactura: 'CR - Electronic Invoice',
      crTiquete: 'CR - Electronic Ticket',
      crNotaDebito: 'CR - Electronic Debit Note',
      ecFactura: 'EC - Invoice',
      ecNotaDebito: 'EC - Debit Note',
      ecNotaCredito: 'EC - Credit Note',
    },
    docFilters: {
      cuitContribuyente: 'Taxpayer CUIT',
      cuitReceptor: 'Receiver CUIT',
      factura: 'Invoice',
      fechaComprobante: 'Receipt Date',
      puntoDeVentas: 'Point of Sale',
      tipoComprobante: 'Receipt Type',
      puntoVenta: 'Point of Sale',
      codigoTipoAutorizacion: 'Authorization Type Code',
      identificador: 'Identifier',
      puntoDeVentasInt: 'Point of Sale',
      cnpjEmissor: 'Issuer CNPJ',
      cnpjReceptor: 'Receiver CNPJ',
      chaveAcesso: 'Access Key',
      serie: 'Series',
      numeroDocumento: 'Document Number',
      codigoNumerico: 'Numeric Code',
      ambiente: 'Environment',
      dataEmissao: 'Issue Date',
      statusSefaz: 'SEFAZ Status',
      statusSefaz2: 'SEFAZ Status 2',
      numMdf: 'MDF Number',
      dtHrEmissao: 'Issue Date/Time',
      numInicial: 'Initial Number',
      cnpj: 'CNPJ',
      cnpjPrestador: 'Provider CNPJ',
      inscricaoMunicipal: 'Municipal Registration',
      numNfse: 'NFS-e Number',
      codigoCancelamento: 'Cancellation Code',
      rutEmisor: 'Issuer RUT',
      tipo: 'Type',
      folio: 'Folio',
      codigoDocumento: 'Document Code',
      fechaEmision: 'Issue Date',
      rutEnvia: 'Sender RUT',
      trackingId: 'Tracking ID',
      referencia: 'Reference',
      tipoOperacion: 'Operation Type',
      tipoLibro: 'Book Type',
      tipoEnvio: 'Sending Type',
      emisorNumeroCedula: 'Issuer ID Number',
      emisorNombre: 'Issuer Name',
      receptorNumeroCedula: 'Receiver ID Number',
      receptorNombre: 'Receiver Name',
      clave: 'Key',
      numeroConsecutivo: 'Consecutive Number',
      rucEmisor: 'Issuer RUC',
      secuencial: 'Sequential',
    },
    docActions: {
      actionAuthorize: 'Authorize',
      actionSend: 'Send',
      actionCancel: 'Cancel',
      actionReceive: 'Receive',
    },
    detail: {
      loading: 'Loading...',
      back: 'Back',
      refresh: 'Refresh',
      actions: 'Actions',
      documentData: 'Document Data',
      semaphore: 'Semaphore',
      tabHistory: 'History',
      tabAttachments: 'Attachments',
      tabErrors: 'Errors',
      tabAudit: 'Audit',
      tabSupport: 'Support',
      tabFlags: 'Flags',
      tabMessages: 'Messages',
      creationDate: 'Creation Date',
      creationDateUTC: 'Creation Date (UTC)',
      attachmentName: 'Attachment Name',
      download: 'Download',
      method: 'Method',
      message: 'Message',
      username: 'Username',
      actionDesc: 'Action',
      description: 'Description',
      flagName: 'Name',
      flagValue: 'Value',
      documentInfo: 'Document Information',
      supportActions: 'Support Actions',
      queueNamePlaceholder: 'Queue name...',
      sendToQueue: 'Send to Queue',
      exportObjectSpace: 'Export ObjectSpace',
      deleteDocument: 'Delete Document',
      confirmDelete: 'Confirm Deletion',
      confirmDeleteMsg: 'Are you sure? This action cannot be undone. Please provide a justification.',
      justification: 'Justification',
      justificationPlaceholder: 'Reason for deletion...',
      cancel: 'Cancel',
      copied: 'Copied!',
      view: 'View',
      copy: 'Copy',
    },

    crud: {
      title: 'CRUD Demo',
      subtitle: 'Demonstration of forms, tables, buttons, cards and other components.',
      totalRecords: 'Total Records',
      activeLabel: 'Active',
      inactiveLabel: 'Inactive',
      monthlyPayroll: 'Monthly Payroll',
      newRecord: 'New Record',
      editRecord: 'Edit Record',
      cancelEdit: 'Cancel Edit',
      name: 'Name',
      email: 'Email',
      phone: 'Phone',
      role: 'Role',
      department: 'Department',
      salary: 'Salary',
      startDate: 'Start Date',
      bio: 'Bio',
      activeToggle: 'Active',
      addRecord: 'Add Record',
      saveChanges: 'Save Changes',
      clear: 'Clear',
      selectPlaceholder: 'Select...',
      namePlaceholder: 'Full name',
      emailPlaceholder: 'email@company.com',
      phonePlaceholder: '(00) 00000-0000',
      salaryPlaceholder: '0.00',
      bioPlaceholder: 'Brief description of the employee...',
      buttonsAndBadges: 'Buttons & Badges',
      records: 'Records',
      filterPlaceholder: 'Filter by name, email or role...',
      actions: 'Actions',
      noRecords: 'No records found.',
      statusActive: 'Active',
      statusInactive: 'Inactive',
      previous: '← Previous',
      next: 'Next →',
      confirmDelete: 'Confirm Delete',
      confirmDeleteMsg: 'Are you sure you want to delete the record of',
      cannotUndo: 'This action cannot be undone.',
      cancel: 'Cancel',
      delete: 'Delete',
      edit: 'Edit',
      badgeNew: 'New',
      badgePending: 'Pending',
      requiredFields: 'Please fill in the required fields: Name, Email and Role.',
      recordUpdated: 'Record for "{name}" updated successfully.',
      recordCreated: 'Record for "{name}" created successfully.',
      recordDeleted: 'Record for "{name}" removed.',
      roles: ['Developer', 'Designer', 'Project Manager', 'Data Analyst', 'QA Engineer', 'DevOps', 'Product Owner'],
      departments: ['Engineering', 'Product', 'Operations', 'Business Intelligence', 'Marketing', 'HR'],
    },
  },

  es: {
    lang: { pt: 'Português', en: 'English', es: 'Español' },
    footer: { rights: 'Todos los derechos reservados.' },
    placeholder: { comingSoon: 'Esta página se implementará próximamente.' },

    config: {
      filters: 'Filtro',
      showFilters: 'Mostrar Filtros',
      company: 'Empresa',
      companyPlaceholder: 'Buscar empresa...',
      noResults: 'Sin resultados',
      documentType: 'Tipo de Documento',
      selectDocType: 'Seleccione...',
      companyCode: 'Código de Empresa',
      selectCode: 'Seleccione...',
      filter: 'Filtrar',
      saveAll: 'Guardar',
      saveIndividual: 'Guardar individual',
      savedOk: 'guardado con éxito.',
      saveError: 'Error al guardar.',
      constantName: 'Descripción',
      value: 'Valor',
    },

    rulePrint: {
      rules: 'Reglas de Impresión',
      addRule: 'Agregar Regla',
      editRule: 'Editar Regla',
      back: 'Volver',
      ruleName: 'Nombre de la Regla',
      printerName: 'Impresora',
      process: 'Proceso',
      paperSource: 'Fuente de Papel',
      copies: 'Copias',
      storePdf: 'Almacenar PDF',
      reportLayout: 'Layout del Reporte',
      pclTemplate: 'Template PCL',
      printerAgent: 'Agente de Impresión',
      xpathCondition: 'Condición XPath',
      xpathPlaceholder: 'Ej: //ide/tpEmis[text()="5"]',
      fileNameOutput: 'Nombre del Archivo',
      selectFile: 'Seleccionar archivo...',
      downloadLayout: 'Descargar Layout',
      downloadPcl: 'Descargar PCL',
      save: 'Guardar',
      actions: 'Acciones',
      edit: 'Editar',
      deleteRule: 'Eliminar',
      noRules: 'No se encontraron reglas.',
      confirmDelete: 'Confirmar Eliminación',
      confirmDeleteMsg: '¿Desea eliminar la regla',
      cancel: 'Cancelar',
      requiredFields: 'Complete los campos obligatorios: Nombre, Impresora y Proceso.',
      ruleCreated: 'Regla creada con éxito.',
      ruleUpdated: 'Regla actualizada con éxito.',
      ruleDeleted: 'Regla eliminada con éxito.',
      ruleExists: 'Ya existe una regla con este nombre.',
      saveError: 'Error al guardar.',
      company: 'Empresa',
      documentType: 'Tipo de Documento',
      companyCode: 'Código de Empresa',
    },

    ruleDist: {
      rules: 'Reglas de Distribución', addRule: 'Agregar Regla', editRule: 'Editar Regla', back: 'Volver', save: 'Guardar',
      ruleName: 'Nombre de la Regla', process: 'Proceso', attachments: 'Adjuntos', fileNameToAttach: 'Nombre de Archivo', extensionsToAttach: 'Extensiones', xpathCondition: 'Condición XPath',
      tabGeneral: 'Regla de Distribución', emailTo: 'Email Para', subject: 'Asunto', templateFile: 'Archivo Template',
      wsUrl: 'URL del WebService', wsHeaderTemplate: 'Template Header', wsBodyTemplate: 'Template Body', wsTransformationTemplate: 'Template Transformación', wsContentTagName: 'Tag de Contenido',
      ftpHostName: 'Host', ftpPortNumber: 'Puerto', ftpProtocol: 'Protocolo', ftpUserName: 'Usuario', ftpPassword: 'Contraseña', ftpPrivateKeyFile: 'Clave Privada', ftpRemoteDirectory: 'Directorio Remoto',
      as2From: 'AS2 De', as2To: 'AS2 Para', as2Url: 'URL AS2', as2PublicKeyFile: 'Clave Pública', as2FromPublicKeyFile: 'Clave Pública Origen',
      download: 'Descargar', actions: 'Acciones', edit: 'Editar', deleteRule: 'Eliminar', noRules: 'No se encontraron reglas.',
      confirmDelete: 'Confirmar Eliminación', confirmDeleteMsg: '¿Desea eliminar la regla', cancel: 'Cancelar',
      requiredFields: 'Complete los campos obligatorios.', ruleCreated: 'Regla creada. Configure los canales en las pestañas.', ruleDeleted: 'Regla eliminada.', ruleExists: 'La regla ya existe.', saveError: 'Error al guardar.', tabSaved: 'Pestaña guardada con éxito.',
      company: 'Empresa', documentType: 'Tipo de Documento', companyCode: 'Código de Empresa',
    },

    partner: {
      partners: 'Socios', addPartner: 'Agregar', editPartner: 'Editar Socio', back: 'Volver', save: 'Guardar',
      name: 'Nombre', actions: 'Acciones', edit: 'Editar', deletePartner: 'Eliminar',
      noPartners: 'No se encontraron socios.', searchPlaceholder: 'Buscar socio...',
      loading: 'Cargando...',
      confirmDelete: 'Confirmar Eliminación', confirmDeleteMsg: '¿Desea eliminar el socio', cancel: 'Cancelar',
      nameRequired: 'El nombre es obligatorio.',
      created: 'Socio creado con éxito.', updated: 'Socio actualizado con éxito.', deleted: 'Socio eliminado con éxito.',
      saveError: 'Error al guardar.', deleteFkError: 'No se puede eliminar: el socio tiene dependencias.',
      hasDependencies: 'Este socio tiene dependencias:', depCompanies: 'empresa(s)', depUsers: 'usuario(s)', depWorkflows: 'workflow(s)',
    },

    company: {
      companies: 'Empresas', addCompany: 'Agregar', editCompany: 'Editar Empresa', back: 'Volver', save: 'Guardar', add: 'Agregar',
      nameLabel: 'Nombre', creationDate: 'Fecha Creación', partnerNameLabel: 'Socio', searchCodeLabel: 'Código Búsqueda', actions: 'Acciones', edit: 'Editar', deleteLabel: 'Eliminar', cancel: 'Cancelar',
      noCompanies: 'No se encontraron empresas.', searchPlaceholder: 'Buscar empresa...', loading: 'Cargando...',
      confirmDelete: 'Confirmar Eliminación', confirmDeleteMsg: '¿Desea eliminar esta empresa?',
      requiredFields: 'Company ID y Nombre son obligatorios.', created: 'Empresa creada.', updated: 'Empresa actualizada.', deleted: 'Empresa eliminada.', saveError: 'Error al guardar.',
      tabCompany: 'Empresa', tabCompanyCode: 'Código Empresa', tabCompanyBranch: 'Sucursal',
      documentType: 'Tipo Documento', codeLabel: 'Código', selectDocType: 'Seleccione el tipo...',
      companyCodePlaceholder: 'Código...', noCodes: 'Sin códigos registrados.', codeAdded: 'Código agregado.', codeDeleted: 'Código eliminado.',
      confirmDeleteCodeMsg: '¿Eliminar código (y todas las configuraciones asociadas)?', cascadeWarning: 'Constantes, reglas de impresión, distribución y contingencia serán eliminadas.',
      branchCodePlaceholder: 'Código sucursal...', branchNamePlaceholder: 'Nombre sucursal...', branchName: 'Nombre',
      noBranches: 'Sin sucursales registradas.', branchAdded: 'Sucursal agregada.', branchUpdated: 'Sucursal actualizada.', branchDeleted: 'Sucursal eliminada.',
      confirmDeleteBranchMsg: '¿Desea eliminar esta sucursal?',
    },

    userPage: {
      users: 'Usuarios', addUser: 'Agregar', editUser: 'Editar Usuario', back: 'Volver', save: 'Guardar',
      group: 'Grupo', name: 'Nombre', phone: 'Teléfono', mobile: 'Celular', password: 'Contraseña', confirmPassword: 'Confirmar Contraseña',
      approved: 'Aprobado', lockedOut: 'Bloqueado', lastLogin: 'Último Login', actions: 'Acciones', edit: 'Editar', deleteUser: 'Eliminar',
      lock: 'Bloquear', unlock: 'Desbloquear', yes: 'Sí', no: 'No',
      noUsers: 'No se encontraron usuarios.', searchPlaceholder: 'Buscar usuario...', loading: 'Cargando...',
      confirmDelete: 'Confirmar Eliminación', confirmDeleteMsg: '¿Desea eliminar el usuario',
      confirmLock: 'Bloquear Usuario', confirmLockMsg: '¿Desea bloquear el usuario',
      confirmUnlock: 'Desbloquear Usuario', confirmUnlockMsg: '¿Desea desbloquear el usuario',
      cancel: 'Cancelar', confirm: 'Confirmar', remove: 'Eliminar',
      created: 'Usuario creado.', updated: 'Usuario actualizado.', deleted: 'Usuario eliminado.', locked: 'Usuario bloqueado.', unlocked: 'Usuario desbloqueado.',
      passwordHint: '10-24 chars: 1 mayúscula, 1 minúscula, 1 número, 1 especial',
      tabUser: 'Usuario', tabCompanyUser: 'Empresas del Usuario',
      selectCompany: 'Seleccione empresa...', addCompany: 'Agregar', companyName: 'Empresa', noCompanies: 'Sin empresas asignadas.',
      companyAdded: 'Empresa asignada.', companyRemoved: 'Empresa eliminada.', companyAlreadyAssigned: 'Empresa ya asignada.',
      errors: { EMAIL_REQUIRED: 'Correo obligatorio.', EMAIL_INVALID: 'Correo inválido.', EMAIL_DUPLICATE: 'Correo en uso.', NAME_REQUIRED: 'Nombre obligatorio.', PHONE_REQUIRED: 'Teléfono obligatorio.', PASSWORD_WEAK: 'Contraseña no cumple requisitos.', PASSWORD_MISMATCH: 'Contraseñas no coinciden.' },
    },

    businessGroup: {
      groups: 'Grupos Empresariales', addGroup: 'Agregar', editGroup: 'Editar Grupo', back: 'Volver', save: 'Guardar',
      name: 'Nombre', description: 'Descripción', actions: 'Acciones', edit: 'Editar', loading: 'Cargando...', noGroups: 'No se encontraron grupos.',
      tabGroup: 'Grupo Empresarial', tabAuthorizations: 'Autorizaciones',
      selectAuthorization: 'Seleccione autorización...', addAuth: 'Agregar', removeAuth: 'Eliminar',
      authName: 'Nombre', authDescription: 'Descripción', noAuthorizations: 'Sin autorizaciones asignadas.',
      allAssigned: 'Todas las autorizaciones fueron asignadas.',
      confirmRemoveAuth: 'Eliminar Autorización', confirmRemoveAuthMsg: '¿Desea eliminar la autorización', cancel: 'Cancelar',
      requiredFields: 'Nombre y Descripción son obligatorios.', created: 'Grupo creado.', updated: 'Grupo actualizado.', saveError: 'Error al guardar.',
      authAdded: 'Autorización agregada.', authRemoved: 'Autorización eliminada.',
    },

    menu: {
      dashboard: 'Dashboard',
      documents: 'Documentos',
      processes: 'Procesos',
      configuration: 'Configuración',
      configGeneral: 'General',
      configRulePrint: 'Regla de Impresión',
      configRuleDistribution: 'Regla de Distribución',
      security: 'Seguridad',
      secPartner: 'Socios',
      secCompanies: 'Empresas',
      secUsers: 'Usuarios',
      secBusinessGroup: 'Grupo Empresarial',
      logReception: 'Log de Recepción',
      idocGeneration: 'Generación de IDoc',
      utilities: 'Utilidades',
      utilSearch: 'Búsqueda y Acciones',
      utilExport: 'Exportación',
      utilCountry: 'Operaciones por País',
      utilQueue: 'Gestión de Colas',
    },
    proc: {
      filters: 'Filtros de Búsqueda', dailyDb: 'Base Diaria', historicalDb: 'Base Histórica', docType: 'Tipo de Documento',
      period: 'Período', dateFrom: 'Fecha creación desde', dateTo: 'Fecha creación hasta', data: 'Datos', serie: 'Serie', numero: 'Número', file: 'Archivo', filePlaceholder: 'IDs (uno por línea)...',
      or: 'O', search: 'Buscar', searchDone: 'Búsqueda completa', found: 'Encontrados', documents: 'documentos', noDocuments: 'No se encontraron documentos.',
      confirmProcess: 'Procesar', colLeft: 'Procesos (Columna 1)', colRight: 'Procesos (Columna 2)',
      queueLabel: 'Cola de Destino', changeQueueTo: 'Cambiar QueueName a', sendToQueue: 'Enviar a Cola', changeTpEmis: 'Cambiar Tipo Emisión',
      exportObjects: 'Exportar Objetos', kelloggIndex: 'Procesar Kellogg', cfdiApproval: 'CFDI Approval Msg',
      clearQueue: 'Clear TFMessageQueue2', downloadNfe: 'Download NFe', rebuildZips: 'Rebuild Zips', grainger: 'Procesar Grainger',
      envioDte: 'Procesar EnvioDTE', exportAttach: 'Exportar Adjunto', dlReturns: 'Descargar Retornos',
      purge: 'Eliminar Documentos', nfePdf: 'NFe PDF Recep', chep: 'Procesar Chep', uuid: 'Procesar UUID', fixTimbre: 'Fix Timbre CFDI',
      logTitle: 'Consola de Procesos', clearLog: 'Limpiar Log',
    },
    util: {
      searchTitle: 'Búsqueda de Documentos', mode_period: 'Por Período', mode_documentId: 'Por Document ID', mode_data: 'Por Datos', mode_file: 'Por Archivo',
      dateFrom: 'Fecha desde', dateTo: 'Fecha hasta', cnpj: 'CNPJ Emisor', serie: 'Serie', numero: 'Número', fileIds: 'IDs (uno por línea)', fileIdsPlaceholder: 'DOC-001\nDOC-002\n...',
      docType: 'Tipo Documento', all: 'Todos', historical: 'Histórico', search: 'Buscar', results: 'Resultados', selected: 'seleccionados',
      noDocsSelected: 'Ningún documento seleccionado.', creationDate: 'Fecha Creación',
      sendToQueue: 'Enviar a Cola', purge: 'Eliminar Documentos', reprint: 'Reimprimir PDF', downloadReturns: 'Descargar Retornos',
      log: 'Consola de Log',
      exportTitle: 'Exportación de Documentos', documentIds: 'Document IDs', idsPlaceholder: 'Pegue los IDs (uno por línea)...',
      noIds: 'Ningún ID proporcionado.',
      exportActions: 'Acciones de Exportación',
      exportObjectSpace: 'Exportar ObjectSpace', exportObjectSpaceDesc: 'Exporta el ObjectSpace completo como XML.',
      exportAttachments: 'Exportar Adjuntos', exportAttachmentsDesc: 'Exporta adjuntos (XML, PDF, TXT).',
      allAttachments: 'Todos los Adjuntos',
      backupExport: 'Backup Completo', backupExportDesc: 'Exporta adjuntos en estructura organizada.',
      exportIndex: 'Índice Tabulado', exportIndexDesc: 'Genera archivo de índice tabulado para CFDI.',
      changeEmissionType: 'Cambiar Tipo Emisión', changeEmissionTypeDesc: 'Cambia tpEmis de NFe, recalcula clave, elimina firma.', justification: 'Justificación de contingencia...',
      chepIndex: 'Procesar CHEP Index', chepIndexDesc: 'Aplica template CHEP y genera adjunto Index.',
      fixTimbreCfdi: 'Fix Timbre CFDI', fixTimbreCfdiDesc: 'Corrige TimbreFiscalDigital en ComprobantePrint.',
      importUuid: 'Importar UUID XMLs', importUuidDesc: 'Importa XMLs con UUID del directorio.',
      cfdiApproval: 'CFDI Approval Message', cfdiApprovalDesc: 'Genera mensaje de aprobación para CFDI.',
      fixEnvioDte: 'Fix EnvioDTE', fixEnvioDteDesc: 'Reconstruye envelope EnvioDTE y firma digitalmente.',
      execute: 'Ejecutar',
      extractObjectSpace: 'Extraer ObjectSpace', extractObjectSpaceDesc: 'Extrae ObjectSpace de una cola de mensajes.',
      queueName: 'Nombre de Cola', count: 'Cantidad', extract: 'Extraer',
      clearQueueTitle: 'Limpiar Cola', clearQueueDesc: 'Elimina todos los mensajes de una cola.', clearQueue: 'Limpiar Cola',
    },

    topbar: {
      search: 'Buscar...',
      newRecord: 'Nuevo Registro',
    },

    dashboard: {
      title: 'Resumen General',
      subtitle: 'Resumen de procesamiento de documentos de los últimos 7 días.',
    },
    dash: {
      totalProcessed: 'Documentos Procesados', totalErrors: 'Errores (7 días)', successRate: 'Tasa de Éxito', avgTime: 'Tiempo Promedio',
      docsPerDay: 'Documentos por Día (7 días)', success: 'Éxito', errors: 'Errores',
      successRateTitle: 'Tasa de Éxito Global',
      errorsByCompany: 'Errores por Empresa (7 días)', company: 'Empresa', other: 'Otros',
      errorsByDocType: 'Errores por Tipo de Documento',
      topDocTypes: 'Volumen por Tipo de Documento', docType: 'Tipo', processed: 'Procesados', successCol: 'Éxito', errorsCol: 'Errores', rate: 'Tasa',
      recentErrors: 'Errores Recientes',
    },

    login: {
      title: 'Bienvenido de nuevo',
      subtitle: 'Inicie sesión para acceder al panel.',
      user: 'Usuario',
      userPlaceholder: 'Ingrese su usuario',
      password: 'Contraseña',
      passwordPlaceholder: 'Ingrese su contraseña',
      submit: 'Ingresar',
      rememberMe: 'Recordarme',
      forgotPassword: '¿Olvidó su contraseña?',
      errorEmpty: 'Complete el usuario y la contraseña.',
      errorInvalid: 'Usuario o contraseña inválidos.',
    },

    docs: {
      filtersTitle: 'Filtros de Documentos',
      clearFilters: 'Limpiar Filtros',
      documentType: 'Tipo de Documento',
      selectDocType: 'Seleccione el tipo...',
      process: 'Proceso',
      selectProcess: 'Seleccione...',
      outbound: 'Documento Enviado (Outbound)',
      inbound: 'Documento Recibido (Inbound)',
      action: 'Acción',
      selectAction: 'Seleccione...',
      situation: 'Situación',
      all: 'Todos',
      processing: 'Procesando',
      finishedSuccess: 'Finalizado con Éxito',
      finishedError: 'Finalizado con Error',
      historic: 'Buscar en histórico',
      search: 'Buscar',
      dynamicFilters: 'Filtros Específicos del Documento',
      from: 'Desde',
      to: 'Hasta',
      radioIgnore: 'Ignorar',
      radioApproved: 'Aprobado',
      radioRejected: 'Rechazado',
      radioNotContains: 'No Contiene',
      results: 'Resultados',
      noResults: 'No se encontraron documentos.',
      createdAt: 'Fecha de Creación',
      showFilters: 'Mostrar Filtros',
      batchActions: 'Acciones en Lote',
      exportExcel: 'Exportar Excel',
      noDocSelected: 'Ningún documento seleccionado.',
    },
    docTypes: {
      arAfipLocalInvoice: 'AR - AFIP Factura Local',
      arAfipLocalInvoiceMtx: 'AR - AFIP Factura Local MTX',
      arAfipExportInvoice: 'AR - AFIP Factura Exportación',
      brNfe: 'BR - NF-e (Nota Fiscal Electrónica)',
      brNf3e: 'BR - NF3e (Energía Eléctrica)',
      brCte: 'BR - CT-e (Transporte)',
      brMdfe: 'BR - MDF-e (Manifiesto)',
      brInutNfe: 'BR - Inutilización NF-e',
      brReprint: 'BR - Reimpresión NF-e',
      brNfseCancelamento: 'BR - NFS-e Cancelación',
      clDte: 'CL - DTE (Documento Tributario)',
      clResultadoEnvio: 'CL - Resultado Envío',
      clResultadoEnvioLibro: 'CL - Resultado Envío Libro',
      clLibroCompraVenta: 'CL - Libro de Compra/Venta',
      crFactura: 'CR - Factura Electrónica',
      crTiquete: 'CR - Tiquete Electrónico',
      crNotaDebito: 'CR - Nota Débito Electrónica',
      ecFactura: 'EC - Factura',
      ecNotaDebito: 'EC - Nota de Débito',
      ecNotaCredito: 'EC - Nota de Crédito',
    },
    docFilters: {
      cuitContribuyente: 'CUIT Contribuyente',
      cuitReceptor: 'CUIT Receptor',
      factura: 'Factura',
      fechaComprobante: 'Fecha Comprobante',
      puntoDeVentas: 'Punto de Ventas',
      tipoComprobante: 'Tipo Comprobante',
      puntoVenta: 'Punto Venta',
      codigoTipoAutorizacion: 'Código Tipo Autorización',
      identificador: 'Identificador',
      puntoDeVentasInt: 'Punto de Ventas',
      cnpjEmissor: 'CNPJ Emisor',
      cnpjReceptor: 'CNPJ Receptor',
      chaveAcesso: 'Clave de Acceso',
      serie: 'Serie',
      numeroDocumento: 'Número de Documento',
      codigoNumerico: 'Código Numérico',
      ambiente: 'Ambiente',
      dataEmissao: 'Fecha de Emisión',
      statusSefaz: 'Estado SEFAZ',
      statusSefaz2: 'Estado SEFAZ 2',
      numMdf: 'Número MDF',
      dtHrEmissao: 'Fecha/Hora Emisión',
      numInicial: 'Número Inicial',
      cnpj: 'CNPJ',
      cnpjPrestador: 'CNPJ Prestador',
      inscricaoMunicipal: 'Inscripción Municipal',
      numNfse: 'Número NFS-e',
      codigoCancelamento: 'Código Cancelación',
      rutEmisor: 'RUT Emisor',
      tipo: 'Tipo',
      folio: 'Folio',
      codigoDocumento: 'Código Documento',
      fechaEmision: 'Fecha Emisión',
      rutEnvia: 'RUT Envía',
      trackingId: 'Tracking ID',
      referencia: 'Referencia',
      tipoOperacion: 'Tipo Operación',
      tipoLibro: 'Tipo Libro',
      tipoEnvio: 'Tipo Envío',
      emisorNumeroCedula: 'Emisor Número Cédula',
      emisorNombre: 'Emisor Nombre',
      receptorNumeroCedula: 'Receptor Número Cédula',
      receptorNombre: 'Receptor Nombre',
      clave: 'Clave',
      numeroConsecutivo: 'Número Consecutivo',
      rucEmisor: 'RUC Emisor',
      secuencial: 'Secuencial',
    },
    docActions: {
      actionAuthorize: 'Autorizar',
      actionSend: 'Enviar',
      actionCancel: 'Cancelar',
      actionReceive: 'Recibir',
    },
    detail: {
      loading: 'Cargando...',
      back: 'Volver',
      refresh: 'Actualizar',
      actions: 'Acciones',
      documentData: 'Datos del Documento',
      semaphore: 'Semáforo',
      tabHistory: 'Historial',
      tabAttachments: 'Adjuntos',
      tabErrors: 'Errores',
      tabAudit: 'Auditoría',
      tabSupport: 'Soporte',
      tabFlags: 'Flags',
      tabMessages: 'Mensajes',
      creationDate: 'Fecha de Creación',
      creationDateUTC: 'Fecha de Creación (UTC)',
      attachmentName: 'Nombre del Adjunto',
      download: 'Descargar',
      method: 'Método',
      message: 'Mensaje',
      username: 'Usuario',
      actionDesc: 'Acción',
      description: 'Descripción',
      flagName: 'Nombre',
      flagValue: 'Valor',
      documentInfo: 'Información del Documento',
      supportActions: 'Acciones de Soporte',
      queueNamePlaceholder: 'Nombre de la cola...',
      sendToQueue: 'Enviar a Cola',
      exportObjectSpace: 'Exportar ObjectSpace',
      deleteDocument: 'Eliminar Documento',
      confirmDelete: 'Confirmar Eliminación',
      confirmDeleteMsg: '¿Estás seguro? Esta acción no se puede deshacer. Proporcione una justificación.',
      justification: 'Justificación',
      justificationPlaceholder: 'Razón de la eliminación...',
      cancel: 'Cancelar',
      copied: '¡Copiado!',
      view: 'Visualizar',
      copy: 'Copiar',
    },

    crud: {
      title: 'CRUD Demo',
      subtitle: 'Demostración de formularios, tablas, botones, tarjetas y otros componentes.',
      totalRecords: 'Total Registros',
      activeLabel: 'Activos',
      inactiveLabel: 'Inactivos',
      monthlyPayroll: 'Nómina Mensual',
      newRecord: 'Nuevo Registro',
      editRecord: 'Editar Registro',
      cancelEdit: 'Cancelar Edición',
      name: 'Nombre',
      email: 'Correo',
      phone: 'Teléfono',
      role: 'Cargo',
      department: 'Departamento',
      salary: 'Salario',
      startDate: 'Fecha de Inicio',
      bio: 'Biografía',
      activeToggle: 'Activo',
      addRecord: 'Agregar Registro',
      saveChanges: 'Guardar Cambios',
      clear: 'Limpiar',
      selectPlaceholder: 'Seleccione...',
      namePlaceholder: 'Nombre completo',
      emailPlaceholder: 'correo@empresa.com',
      phonePlaceholder: '(00) 00000-0000',
      salaryPlaceholder: '0,00',
      bioPlaceholder: 'Breve descripción del colaborador...',
      buttonsAndBadges: 'Botones & Badges',
      records: 'Registros',
      filterPlaceholder: 'Filtrar por nombre, correo o cargo...',
      actions: 'Acciones',
      noRecords: 'No se encontraron registros.',
      statusActive: 'Activo',
      statusInactive: 'Inactivo',
      previous: '← Anterior',
      next: 'Siguiente →',
      confirmDelete: 'Confirmar Eliminación',
      confirmDeleteMsg: '¿Estás seguro de que deseas eliminar el registro de',
      cannotUndo: 'Esta acción no se puede deshacer.',
      cancel: 'Cancelar',
      delete: 'Eliminar',
      edit: 'Editar',
      badgeNew: 'Nuevo',
      badgePending: 'Pendiente',
      requiredFields: 'Complete los campos obligatorios: Nombre, Correo y Cargo.',
      recordUpdated: 'Registro de "{name}" actualizado con éxito.',
      recordCreated: 'Registro de "{name}" creado con éxito.',
      recordDeleted: 'Registro de "{name}" eliminado.',
      roles: ['Desarrolladora', 'Diseñador', 'Gerente de Proyectos', 'Analista de Datos', 'QA Engineer', 'DevOps', 'Product Owner'],
      departments: ['Ingeniería', 'Producto', 'Operaciones', 'Business Intelligence', 'Marketing', 'RRHH'],
    },
  },
}

export default translations
